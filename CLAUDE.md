...
- `supabase/migrations/` 0001 à 0019, toutes appliquées en production au 5 septembre 2026.
...
## État au 5 septembre 2026, au soir

Quarante-deux PR fusionnées. Les phases 0 à 7 de la feuille de route sont livrées côté dépôt.
La migration 0019 est appliquée. Le domaine `cloison.immo` est rattaché à Vercel, avec ses DNS
chez Cloudflare, et les fonctions répondent depuis `fra1`. Resend envoie depuis le domaine
vérifié. Stripe est en mode réel : compte activé, IBAN posé, clés dans Vercel, webhook déclaré,
et un tour complet de paiement vérifié par Lounes le 5 septembre 2026.
...
Content-Security-Policy est posée, l'espace agence affiche l'adresse de support dès qu'elle
existe, le webhook ne répond 200 qu'à ce qui est réglé pour de bon, et l'ancien hôte
`cloison.vercel.app` redirige vers le domaine. Trois dettes fermées : PostgREST, CSP, réponse
oracle.

Ce qui reste ne se code pas, et se fait de son côté :

1. **Choisir l'hôte principal**, apex ou `www` : Vercel a fait de `www.cloison.immo` l'hôte
   principal et l'apex redirige dessus. Si tu inverses, refaire dans le même mouvement l'URL de
   redirection Supabase (`/connexion/verifie`) et l'URL du webhook Stripe, qui ne suit pas une
   redirection.
2. **Le courrier entrant** : chez Cloudflare, terminer Email Routing pour que `support@cloison.immo`
   arrive dans ta boîte, l'enregistrement MX de l'apex manque encore ; ajouter un TXT `_dmarc`
   (`v=DMARC1; p=none; rua=mailto:...`) pour que les courriels de Resend ne finissent pas en
   indésirables. Puis poser `EMAIL_SUPPORT` dans Vercel.
...
8. Le jour du point hebdomadaire et l'endroit du compte rendu.
9. Supprimer depuis GitHub les branches fusionnées qui restent sur le dépôt distant : la
   suppression échoue depuis la session cloud.
